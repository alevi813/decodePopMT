...
function [sessionStruct] = sessionDecoding(model, condition, fitWindow, plotWindow)
...
%     if strcmp(model, 'choice')
%         savePath   = ['/Users/aaronlevi/Dropbox/twagAnalysis4.1/decoding/sessionData/no_xval/choice/' fitWindow 'Fit_' plotWindow 'Plot'];
%     else
%         savePath   = ['/Users/aaronlevi/Dropbox/twagAnalysis4.1/decoding/sessionData/no_xval/direction/' fitWindow 'Fit_' plotWindow 'Plot'];
%     end
...
for kEx = 1:numel(experiments)
    ...
    for kNeuron = 1:nNeurons
        ...
        %badFR(kNeuron) = max(nanmean(fromGo.sprate))<5.01;
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    end % neuron loop
    
    % trying things out with removal of low firing rate neurons for the
    % "fromGo" model
    %motionOn.Rs(:,:,badFR) = [];
    %fromGo.Rs(:,:,badFR) = [];
    ...
    loo_avgW = mean( cell2mat(arrayfun(@(x) x.wTrain , sLoo, 'UniformOutput', false)), 2 );
